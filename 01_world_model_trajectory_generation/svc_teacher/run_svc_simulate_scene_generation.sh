...
VERSION="${VERSION:-svc_simulate_demo}"
SIMULATE_CONFIG="${SIMULATE_CONFIG:-${SCRIPT_DIR}/config/svc_teacher_generation.yaml}"
...
if [[ ! -s "${MANIFEST}" ]]; then
  ...
  if [[ "${#IMAGE_FILES[@]}" -eq 0 ]]; then
    echo "No input images found under ${INPUT_DIR}" >&2
    exit 1
  fi
