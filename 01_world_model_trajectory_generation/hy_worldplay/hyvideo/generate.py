...
def resolve_generation_task(image_path):
    return "i2v" if image_path else "t2v"


def resolve_transformer_dtype(dtype):
    if dtype == "bf16":
        return torch.bfloat16
    if dtype == "fp32":
        return torch.float32
    raise ValueError(f"Unsupported dtype: {dtype}. Must be 'bf16' or 'fp32'")


def create_generation_pipeline(args):
    initialize_infer_state(args)

    task = resolve_generation_task(args.image_path)
    ...
    return HunyuanVideo_1_5_Pipeline.create_pipeline(
        ...
        transformer_dtype=resolve_transformer_dtype(args.dtype),
...
def run_generation_once(
    ...
    *,
    ...
    task = resolve_generation_task(image_path)
    extra_kwargs = {}
    if task == "i2v":
...
def select_primary_video_tensor(out, enable_sr):
    if enable_sr and hasattr(out, "sr_videos"):
        return out.sr_videos
    return out.videos


def save_generation_outputs(out, args):
    if int(os.environ.get("RANK", "0")) != 0:
        return {}
...
def generate_video(args):
    ...
    pipe = create_generation_pipeline(args)
    out = run_generation_once(
        pipe,
        prompt=args.prompt,
    ...
    save_generation_outputs(out, args)
    return out
